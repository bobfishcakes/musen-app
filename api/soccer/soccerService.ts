...
class SoccerService extends BaseApiService {
    ...
    async getGames(date: string): Promise<SoccerGame[]> {
        console.log('=== SoccerService.getGames Start ===');
        ...
        try {
            ...
            const responses = await Promise.all(
                dates.map(async (dateStr) => {
                    ...
                    try {
                        const response = await this.get<SoccerApiResponse<SoccerGame>>('/fixtures', { date: dateStr });
            ...
            const filteredGames = allGames.filter(game => {
                const gameTime = new Date(game.fixture.date);
            ...
            console.log('=== SoccerService.getGames End ===');
            return sortedGames;

        } catch (error) {
            console.error('Error in SoccerService.getGames:', {
